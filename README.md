...
2. **OSSM v3 실습 모듈 작성 및 검증:**
   ...
   - 한국어 번역 및 로컬라이징(Localized) 실습 가이드북 완비
     ...
     7) 본문 중, 웹콘솔이나 어플리케이션 url등을 제공할 경우에는 하이퍼링크로 새탭에 열릴 수 있게 제공합낟.
     8) lab1.3의 제일 상위에 지정된 style이나 .app을 삭제하는 스크립트는 모두 기본으로 기입한다.
     9) pdf내용 중, 빨간원의 숫자로 표시된 부분(특정 yaml의 value를 설명할 때 많이 사용)과 그에 따른 설명 부분을 동일하게 표현할 수 있도록 한다.(방법을 찾아서)

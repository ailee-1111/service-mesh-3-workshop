...
### 🖥️ 오픈시프트 웹 콘솔 접속 안내 표준 (Web Console Tab)
가이드북 마크다운 내에서 오픈시프트 웹 콘솔(`console-openshift-console`)에 대한 접속이나 사용자 관점 전환 지침이 나타날 때에는, 브라우저의 새 창/새 탭을 직접 열도록 하는 전통적인 지시 대신 **가이드 대시보드 인터페이스 상단 메뉴바의 'Web Console' 탭을 마우스로 클릭해 원스톱으로 연동 접속하여 실습할 수 있도록 친절한 안내 문구를 삽입**해 주어야 합니다. (향후 작성될 모든 가이드북 페이지 공통 표준)
...
* **`workshop/modules.yaml` (메뉴 제목 정의):** 가이드북 좌측 카탈로그에 표시될 다채롭고 한글화된 제목 사양을 작성합니다.
  * 예:
    ...
    ```
* **모듈 헤더 타이틀 명명 표준:** 각 실습 마크다운 문서 최상단의 메인 제목 H1 헤더는 일관성을 위해 반드시 `# 모듈 X.Y: [실습명] ([영문실습명])` 포맷을 엄격하게 사용해 정의해야 합니다. (예: `# 모듈 1.3: 서비스 메시 쇼룸 애플리케이션 (Service Mesh Showroom Application)`)

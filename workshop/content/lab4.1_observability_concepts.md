...
## Integrating OpenShift Service Mesh Projects(프로젝트) in Default OpenShift Observability
...
* **서비스 토폴로지 라이브 가시화 (Service(서비스) Topology(토폴로지) Visualization):**
  Kiali는 분산된 마이크로서비스 간에 트래픽이 교차 소화되는 과정을 실시간 입체 그래프(Topology(토폴로지) Graph) 차트로 동적 그려냅니다. 이 그래프 선상을 통해 다음 성능 정보들을 직관 스캐닝할 수 있습니다:
...
* **L7 설정 자산 통제 관리 (Configuration Management):**
  ...
  - VirtualService(가상 서비스) 및 DestinationRule(대상 규칙) 자산 명세
  - Gateway(게이트웨이) 인프라 포트 연동 명세
...
<img src="images/lab4.1-observability-fig-001.svg" width="100%" alt="Figure 1.1: Integration of Kiali and OSSMC with OpenShift observability components" />
...
❶ Kiali 서버를 설치 구동하고 감시할 이스티오 제어 평면 타깃 네임스페이스를 선언합니다.<br/>
❷ Kiali 로그인 인증 단계를 오픈시프트 전역 Oauth 표준 SSO 인증망과 결합하도록 셋업 합니다.<br/>
❸ Kiali 관제판이 배후에서 실시간 조회 수렴할 그라파나, 프로메테우스, 템포 등의 원격 시계열 API 주소 정보들을 연대 매핑 기입해 넣습니다.<br/>
...
<img src="images/lab4.1-observability-fig-002.png" width="100%" alt="Service Mesh entry in Administrator(관리자) perspective" />

<img src="images/lab4.1-observability-fig-002-1.png" width="100%" alt="Service Mesh entry in Administrator(관리자) perspective2" />
...
* **도형 및 선로 레전드표 (Graph Legend):**
  - Kiali는 노드가 감당하는 비즈니스 성질에 따라, 노드 도형의 기하학적 형태(Workload, App, Operation, Service(서비스), Service(서비스) Entry 등)와 테두리 컬러 감도(Green: 정상, Yellow/Orange: 지체 경고, Red: 에러 임계 초과 위험 등)를 다르게 렌더링하여 관제사에게 즉각적인 직관 지표를 하사합니다.
...
* **그래프 렌더링 세분화 4대 등급 (Graph Types):**
  ...
  - **Service(서비스) graph:** 오직 쿠버네티스 서비스(`Service(서비스)`) 가상 도메인 엔드포인트 노선 간의 패킷 유입 정황만을 단순화해 표출합니다.

...
Go to `https://console-openshift-console.apps.ocp4.example.com` (또는 실제 본인의 클러스터 도메인인 `https://console-openshift-console.%cluster_subdomain%`), click `htpasswd_provider` and use `developer` as the user and the password. 가이드 투어(guided tour)가 표시되면 Skip tour를 클릭합니다.
...
1.3. **Topology** 메뉴 항목을 클릭하여 토폴로지 뷰를 열고 배포를 검사합니다. 프로젝트 목록 드롭다운에서 `meshintro-bookinfo` 프로젝트가 선택되어 있는지 확인합니다.
...
Go to `http://istio-ingressgateway-istio-ingress.apps.ocp4.example.com/productpage` (또는 실제 본인의 클러스터 도메인인 `http://istio-ingressgateway-istio-ingress.%cluster_subdomain%/productpage`).
...
2.2. 새 터미널 창에서 developer 사용자명과 developer 비밀번호로 로그인하고, `meshintro-bookinfo` 프로젝트로 전환합니다:
...
`meshintro-bookinfo` 프로젝트의 분당 평균 요청 수가 약 200건인 것을 확인하십시오. 본인의 값은 다르게 나타날 수 있습니다.

3.3. **Traffic Graph** 메뉴 항목을 클릭하고 상단의 **Select Namespaces** 콤보박스에서 `meshintro-bookinfo` 프로젝트만 선택합니다.
